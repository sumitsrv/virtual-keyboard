...
IplImage Haar::haar(IplImage *img, IplImage *gray) {
  int depth = img->depth;
  ...
  IplImage *downscaled =
      cvCreateImage(cvSize(sz.width / 2, sz.height / 2), depth, nchannels);
  IplImage *upscaled =
      cvCreateImage(cvSize(sz.width, sz.height), depth, nchannels);
...
void Haar::haar(IplImage *img, IplImage *copy, int scale) {
  int width = img->width;
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      for (k = 0; k < nchannels; k++) {
        ...
        arr[(i / 2) * step + (j / 2) * nchannels + k] =
            (0.5) * (intensity[0] + intensity[1] + intensity[2] + intensity[3]);
        if (arr[(i / 2) * step + (j / 2) * nchannels + k] >= max[k]) {
          max[k] = arr[(i / 2) * step + (j / 2) * nchannels + k];
        }
        // data[(i/2)*step + (j/2)*nchannels + k] = (0.25)*(intensity[0] +
        // intensity[1] + intensity[2] + intensity[3]);
        data[(i / 2) * step + ((width / 2) + (j / 2)) * nchannels + k] =
            (0.5) *
            (abs(intensity[0] - intensity[1] + intensity[2] - intensity[3]));

        data[(height / 2 + i / 2) * step + (j / 2) * nchannels + k] =
            (0.5) *
            (abs(intensity[0] + intensity[1] - intensity[2] - intensity[3]));
        data[(height / 2 + i / 2) * step + ((width / 2) + (j / 2)) * nchannels +
             k] =
            (0.5) *
            (abs(intensity[0] - intensity[1] - intensity[2] + intensity[3]));
      }
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      for (k = 0; k < nchannels; k++) {
        data[(i / 2) * step + (j / 2) * nchannels + k] =
            (arr[(i / 2) * step + (j / 2) * nchannels + k] * 255 / max[k]);
      }
      j++;
    }
    i++;
...
void Haar::make_icon(IplImage *src, IplImage *dest, int scale) {
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      for (k = 0; k < nchannels; k++) {
        if (data[i * step + j * nchannels + k] <=
            srcData[i * (2 * step) + (j + width) * nchannels + k]) {
          data[i * step + j * nchannels + k] =
              srcData[i * (2 * step) + (j + width) * nchannels + k];
        }
      }
    }
...
void Haar::up_Scale(IplImage *img, IplImage *upscale, int scale) {
  int width = img->width;
  int height = img->height;
  int step = img->widthStep;
  int nchannels = img->nChannels;
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      for (k = 0; k < nchannels; k++) {
        ...
        for (p = 0; p < scale; p++) {
          for (q = 0; q < scale; q++) {
            newdata[((scale * i) + p) * step * scale +
                    ((scale * j) + q) * nchannels + k] = intensity;
          }
        }
      }
    }
