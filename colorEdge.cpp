  ...
  for (i = 1; i < height - 1; i++) {
    for (j = 1; j < width - 1; j++) {
      ...
      for (k = 0; k < nchannels; k++) {
        u[k] = (data[(i + 1) * step + (j - 1) * nchannels + k] +
                2 * data[(i + 1) * step + j * nchannels + k] +
                data[(i + 1) * step + (j + 1) * nchannels + k]) -
               (data[(i - 1) * step + (j - 1) * nchannels + k] +
                2 * data[(i - 1) * step + j * nchannels + k] +
                data[(i - 1) * step + (j + 1) * nchannels + k]);
        v[k] = (data[(i - 1) * step + (j + 1) * nchannels + k] +
                2 * data[i * step + (j + 1) * nchannels + k] +
                data[(i + 1) * step + (j + 1) * nchannels + k]) -
               (data[(i - 1) * step + (j - 1) * nchannels + k] +
                2 * data[(i)*step + (j - 1) * nchannels + k] +
                data[(i + 1) * step + (j - 1) * nchannels + k]);
      ...
      copy_data[i * step / nchannels + j] =
          pow(((0.5) * ((g_xx + g_yy) + ((g_xx - g_yy) * cos(2 * theta)) +
                        (2 * g_xy * sin(2 * theta)))),
              0.5);
    }
  }
