...
HandDetect::HandDetect(WebCam& webcam) {
    webcam.subscribeToCamStream(boost::bind(&HandDetect::getKeyPress, this, _1));
}

void HandDetect::getKeyPress(Mat frame) const {
  ...
  //      if (area > 500) {
  //        drawContours(handview, contours, i, Scalar(255), 1, 8, noArray(),
  //        0);
  //      }
  //    }

  //    int tipCount = getTipsCount(handview, handview.cols, handview.rows);

  //    for (i = 0; i < tipCount; i++) {
  //      if (posmax == i)
  //        circle(src, Point(tips_position[posmax][1],
  //        tips_position[posmax][0]),
  //               3, Scalar(0, 255, 0), 2, 8, 0);
  //      else
  //        circle(src, Point(tips_position[i][1], tips_position[i][0]), 3,
  //               Scalar(255, 0, 0), 2, 8, 0);

  //      if (speed[i][1] > 1 && speed[i][2] > 1 && (int)speed[i][5] == 1 &&
  //          oldtips[(oldtipflag + 1) % FRAMERUN][i][2] == 1) {
  //        circle(src, Point(speed[i][4], speed[i][3]), 5, Scalar(0, 0, 255),
  //        3, 8,
  //               0);
  //        speed[i][1] = 0;
  //        speed[i][2] = 0;
...
int HandDetect::getTipsCount(Mat img, int width, int height) {

  FILE *tipgraph = fopen("tipgraph.dat", "w");

  size_t i, j, curr_pixel, step, x[640];
  int tipcount = 0;
  uchar *data = img.data;
  step = img.step;
      ...
      if (distx <= 300 && disty <= 300) {
        if (oldtips[oldtipflag][j][0] - tips_position[i][0] != 0)
          oldtips[(oldtipflag + 1) % FRAMERUN][j][2] =
              (oldtips[oldtipflag][j][0] - tips_position[i][0]) /
              abs(oldtips[oldtipflag][j][0] - tips_position[i][0]);
        else
          oldtips[(oldtipflag + 1) % FRAMERUN][j][2] =
