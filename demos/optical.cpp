...

{
	...
	if (argc > 1) {
	 ...
	 #ifdef _WIN32
	    	 input_video = cvCaptureFromCAM(-1); // capture from video device
	 #else
		  	 input_video = cvCaptureFromCAM(0); // capture from video device #i
	  
	       // Add in the following 2 lines for Quickcam Express II on linux

     		// cvSetCaptureProperty(input_video, CV_CAP_PROP_FRAME_WIDTH, 352);
     		// cvSetCaptureProperty(input_video, CV_CAP_PROP_FRAME_HEIGHT, 288);
	 #endif
...

	{
		...
		fprintf(stderr, "Error: Can't open video / camera.\n");
	...
	/* Read the video's frame size out of the AVI / camera */

	IplImage *tmp = cvQueryFrame( input_video );
	
	CvSize frame_size;

	frame_size.height = tmp->height;

	frame_size.width = tmp ->width;

	
	printf("VIDEO : width %i x hieght %i\n", frame_size.width, frame_size.height );
...

	{
		 ...
		 * in AVIs upside-down by default (ON WINDOWS).  (No comment :-))
		...
		CvPoint2D32f frame1_features[2000];
		...
		CvPoint2D32f frame2_features[2000];
		...
		char optical_flow_found_feature[2000];
		...
		float optical_flow_feature_error[2000];
