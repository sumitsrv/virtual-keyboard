...
void Contrast::glg(Mat img)
{
    ...
    Mat gray = Mat(Size(sz.width, sz.height), 8, 1);
    Mat copy = Mat(Size(sz.width, sz.height), 8, 3);

    Mat(img, Rect(0, 0, sz.width, sz.height));

    if(img.channels() == 3)
    {
        cvtColor(img, copy, CV_BGR2HSV, 0);
        //            SetImageCOI(copy, 3);
        gray = copy.clone();
    }
    else
    ...
    calcHist(&img, 1, &channels, noArray(), hist, 1, &hist_size, &ranges, false);
    ...
    for(i=n-40; i>0; i--)
    {
        ...
        for( s = 0; s < n; s++ )
        {
            float bin_val = hist_glg.at<float>(s);
            int intensity = cvRound(bin_val*255/max_value);
            //printf("%f\t",bin_val);
    ...
    if(img.channels() == 3)
    {
        ...
        cvtColor(copy, img, CV_HSV2BGR, 0);
    }
    else
    {
        img = gray.clone();
...
void Contrast::mergeLevels(Mat hist_glg, float *max_value, int *hist_size, float left[], float right[])
{
    ...
    {
        float bin_val = hist_glg.at<float>(n);
    ...
    if(index>0 && index<(*hist_size)-1)
    {
        ...
        val_next = hist_glg.at<float>(index + 1);
