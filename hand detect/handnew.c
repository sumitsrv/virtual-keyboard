...
int tips_position[10][2], tipcount=0, oldtips[FRAMERUN][11][4], oldtipflag=0;
int  posmax=0;	
...
int main()
{
	...
	CvScalar  hsv_min = cvScalar(0, 30, 0, 0);
	CvScalar  hsv_max = cvScalar(30, 150, 255, 0);
	...
	while( c != 27)
	{
		...
		for( ; contour != 0; contour = contour->h_next )
		{
			...
			if(area > 500)
			{
				cvDrawContours( handview, contour, cvScalar( 255 ), cvScalar( 255 ), 0, CV_FILLED, 8 );
				//max = area;
			}
		}
		...
		for(i=0; i<tipcount; i++)
		{
			...
			//printf("%d\t", (int)speed[i][5]);
			/*
			if(speed[i][1] > 2 && speed[i][2] > 1 && (int)speed[i][5] == 1 && oldtips[(oldtipflag+1)%FRAMERUN][i][2] == 1)
			{	
			...
			else if(speed[i][1] > 1 && speed[i][2] > 1 && (int)speed[i][5] == 1 && oldtips[(oldtipflag+1)%FRAMERUN][i][2] == -1)
			{	
